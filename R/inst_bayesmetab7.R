...
data.dir <- here('BASEmetab-res/input7')
results.dir <- here('BASEmetab-res/output7')
...
p1 <- ggplot(daily, aes(x = Date, y = val, color = var)) + 
  ...
  labs(
    ...
    x = NULL, 
    color = NULL, 
    title = 'Dry Bar May 2022, BASemetab'
  )
...
png('figs/apadbMay2022BASEmetab.png', height = 5, width = 7, res = 300, units = 'in')
print(p)
dev.off()
...
write.csv(dat, 'data/apadbMay2022hourlyBASemetab.csv', row.names = F)